...
class Cell:
    ...
    def draw(self, canvas: Canvas, fill_color: str):
        ...
        fill_color_left = fill_color if self.has_left_wall else BLANK
        canvas.create_line(
            self._x1, self._y1, self._x1, self._y2, fill=fill_color_left, width=2
        )

        fill_color_right = fill_color if self.has_right_wall else BLANK
        canvas.create_line(
            self._x2, self._y1, self._x2, self._y2, fill=fill_color_right, width=2
        )

        fill_color_top = fill_color if self.has_top_wall else BLANK
        canvas.create_line(
            self._x1, self._y1, self._x2, self._y1, fill=fill_color_top, width=2
        )

        fill_color_bottom = fill_color if self.has_bottom_wall else BLANK
        canvas.create_line(
    ...
    def draw_move(self, to_cell: "Cell", undo: bool = False):
        if self._win is None:
            raise ValueError("Window is not provided")
        color = "gray"
        if not undo:
            color = "red"
